// Add a Recipe Part (Global Query Selectors)// 
...
function addNoteToList(note) {
  ...
  newUINote.innerHTML = `
    ...
    <div class="note__btns">
      <button class="note__btn note__view">View a recipe</button>
      <button class="note__btn note__delete">Delete Recipe</button>
    </div>
  `;
...
noteContainer.addEventListener('click', (e) => {
  ...
  if(e.target.classList.contains('note__delete')){
    const currentNote = e.target.closest('.note');
    showAlertMessage('Your recipe was permanently deleted', 'remove-message');
    currentNote.remove();
...
form.addEventListener('submit', (e) => {
  ...
  if(titleInput.value.length > 0 && noteInput.value.length > 0){
    ...
    titleInput.value = '';
    noteInput.value = '';
    showAlertMessage('Recipe successfully added', 'success-message');
    titleInput.focus();
  } else {
    showAlertMessage('Please add both a title and a recipe', 'alert-message');
  }
});
